...
fn pipe() -> Result<(c_int, c_int), c_int> {
    ...
    loop {
        ...
        let result = unsafe { libc::pipe((&mut fds).as_mut_ptr(), 128, libc::O_TEXT) };
...
fn dup2(src: c_int, dst: c_int) -> Result<(), c_int> {
    ...
    loop {
        ...
        if result >= 0 {
            if result != dst {
                let errno = errno::errno().0;
                let errno = if errno == 0 { libc::ENOSYS }
                else { errno };
...
fn read(fd: c_int, buf: &mut [u8]) -> Result<usize, c_int> {
    #[cfg(any(target_family="windows", target_family="unix"))]
    loop {
        let result = unsafe { libc::read(fd, std::mem::transmute(buf.as_mut_ptr()), buf.len() as libc::size_t) };
        if result >= 0 {
...
fn write_all(fd: c_int, mut buf: &[u8]) -> Result<(), c_int> {
    #[cfg(any(target_family="windows", target_family="unix"))]
    while !buf.is_empty() {
        let result = unsafe { libc::write(fd, std::mem::transmute(buf.as_ptr()), buf.len() as libc::size_t) };
        if result >= 0 {
...
pub(crate) fn attempt_stderr_capture(output: crate::Output) {
    ...
    if !std::io::stderr().is_tty() { return }
    let (r, w) = match pipe() {
        Ok(x) => x,
        Err(x) => {
            let _ = output.tx.send(crate::Request::StderrLine(
                format!("pipe() returned error {:?} when attempting to capture stderr.", x)
            ));
            return
        },
    };
    let real_stderr = match dup(STDERR_FD) {
        ...
        Err(x) => {
            let _ = output.tx.send(crate::Request::StderrLine(
                format!("dup(STDERR_FD) returned error {:?} when attempting to capture stderr.", x)
            ));
            return
        },
    };
    if let Err(x) = dup2(w, STDERR_FD) {
        ...
        close(w);
        let _ = output.tx.send(crate::Request::StderrLine(
            format!("dup2() returned error {:?} when attempting to capture stderr.", x)
        ));
        return
    }
    ...
    *lock = Some(std::thread::spawn(move || {
        ...
        'outer: loop {
            ...
            match read(r, &mut buf[buf_pos..]) {
                Ok(0) => {
                    // stderr ended?!
                    if buf_pos > 0 {
                        if let Err(_) = output.tx.send(crate::Request::StderrLine(
                            String::from_utf8_lossy(&buf[..buf_pos]).to_string()
                        )) {
                ...
                Ok(x) => {
                    ...
                    while let Some(p) = buf[buf_pos..].iter().position(|x| *x == b'\n').map(|x| x + buf_pos) {
                        let start_pos = last_newline_pos.map(|x| x+1).unwrap_or(0);
                        if let Err(_) = output.tx.send(crate::Request::StderrLine(
                            String::from_utf8_lossy(&buf[start_pos..p]).to_string()
                        )) {
                            // can't do anything sensible with an error here
                            let _ = write_all(real_stderr, &buf[start_pos..end_pos]);
                            buf_pos = 0;
                            break 'outer
                        }
                ...
                Err(x) => {
                    ...
                    let _ = output.tx.send(crate::Request::StderrLine(
                        format!("read() returned error {:?} when reading from stderr.", x)
                    ));
                    buf_pos = 0;
                    break
                },
        ...
        assert_eq!(buf_pos, 0, "INTERNAL LISO ERROR: buf contents not fully handled when liso closed down!");
        ...
        dup2(real_stderr, STDERR_FD).expect("Unable to reduplicate stderr back into place!");
        close(real_stderr);
