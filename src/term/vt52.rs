...
fn input_thread(input_rx: std_mpsc::Receiver<Vec<u8>>,
                req_tx: std_mpsc::Sender<Request>) -> LifeOrDeath {
    let mut buf = Vec::new();
    loop {
        ...
        while start < buf.len() {
            ...
            if buf[start] == ESCAPE {
                ...
                let text = String::from_utf8_lossy(&buf[start..text_end])
                    .to_string();
...
impl Vt52 {
    pub(crate) fn new(req_tx: std_mpsc::Sender<Request>, num_colors: u8)
    -> Result<Vt52, DummyError> {
        let white_on_black = match std::env::var("ATARI_WHITE_ON_BLACK")
            .as_ref().map(String::as_str) {
                ...
                Ok(_) => {
                    eprintln!("Unrecognized value for ATARI_WHITE_ON_BLACK \
    ...
    fn set_attrs(&mut self, style: Style,
        ...
        let (fg, bg) = if self.white_on_black {
            match self.num_colors {
                16 => (fg.map(Color::to_atari16_bright).unwrap_or(0),
                       bg.map(Color::to_atari16_dim).unwrap_or(15)),
                4 => (fg.map(Color::to_atari4).unwrap_or(0),
            ...
            match self.num_colors {
                16 => (fg.map(Color::to_atari16_dim).unwrap_or(15),
                       bg.map(Color::to_atari16_bright).unwrap_or(0)),
                4 => (fg.map(Color::to_atari4).unwrap_or(15),
                      bg.map(Color::to_atari4).unwrap_or(0)),
                2 => (15, 0),
                _ => unreachable!(),
            }
        };
        let (fg, bg) = if style.contains(Style::INVERSE) { (bg, fg) }
        else { (fg, bg) };
